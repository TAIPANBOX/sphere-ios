...
| C9 | ~~**SphereUI localization (uk)** — known debt, strings in `app_uk.arb`~~ DONE (2026-07-06): `SphereCore/Sources/SphereUI/Localizable.xcstrings` (604 keys, en source + uk translations) added; `defaultLocalization: "en"` and explicit `resources: [.process("Localizable.xcstrings")]` on the SphereUI target in Package.swift (xcstrings is NOT auto-processed by SwiftPM — confirmed experimentally). Trap confirmed: plain `Text("literal")`/`Button("literal", ...)`/`Label`/`TextField`/`Section`/`.navigationTitle` resolve against `Bundle.main`, not the package's `Bundle.module` — verified by forcing the iOS Simulator to the uk locale and watching a SphereUI string render in English while the app-shell catalog's strings correctly localized. Fixed via two helpers in `SphereUI/Localization.swift`: `Text(ui:)` (routes `Text(_:bundle:)` through `.module`) and `uiString(_:)` (routes `String(localized:bundle:)` through `.module` for the `String`-typed component params like `EmptyStateCard`). Applied across all 32 SphereUI files (12 sphere screens, Home, Components, Settings, Chat). Verified end-to-end: `xcodebuild` succeeds, and a uk-locale simulator screenshot shows package strings (e.g. "Tell your agent anything" → "Розкажи своєму агенту що завгодно") correctly localized post-fix. `SphereCoreTests/SphereUILocalizationTests.swift` guards the resource wiring (catalog ships in `Bundle.module`, known keys have `uk` translations) — note `swift test`'s SwiftPM-CLI build copies the xcstrings uncompiled (no lproj), unlike Xcode's build system, so the test asserts catalog content directly rather than through `Bundle.localizations`. Compromises: no xcstrings plural `variations` (flat translated strings for count-bearing text, e.g. "%lld active"); a handful of Chart axis/legend labels and one multi-placeholder health format string left un-cataloged; `Momentum.progressPhrase`/`ReadinessEngine` computed strings (SphereCore data layer, not SphereUI) intentionally out of scope. | Mechanical, big volume | P1 |
...
8. ~~C9 SphereUI uk localization~~ DONE (2026-07-06)
